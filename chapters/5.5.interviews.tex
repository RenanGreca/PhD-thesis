...
Information about how testing is performed at \sugg{the industrial partner} and regarding the challenges still faced at the company was gathered via a series of interviews.
...
The interviewed team members are mostly working on MCT and have all mentioned two key processes for this layer: \quoter{[one] for delivery, [another] for nightly runs}{R1}. \todo{There is a mention of nightly run in the quote, should we rephrase this?}
\sugg{As previously discussed, we use the acronym SIRT to refer to a short execution of the test suite that happens whenever a developer pushes changes to be merged into the main branch of a module.}
...
\sugg{The LIRT is also known as ``assessment loop'', which includes all tests in the suite, and can take up to 10 hours.}
...
R4 explains: \quotes{we had attempts to integrate \sugg{[the test tool mentioned by R8]} but it did not work out so well. I think we never built a business case for [the new version of the system] and it ended up way in the backlog. In 2017 or 2018 we said `this is something we should add to our plan', but we did not do anything.} 
...
Continuing on this line of thought, some respondents mention there is some collaborative projects that happen \sugg{involving the company and a nearby university}.
...
R7, however, mentions volunteering \quotes{a study \sugg{by another company [about] roles for testers, [...] I think [the researcher] is employed by both that company and the local university}}, indicating there is some cooperation happening \sugg{between the two companies and the local university}.
