...
Since the mid-2000s, \sugg{the company in question} has adopted agile development principles.
...
Currently, \sugg{their test design} is based on a multi-layer ``test strategy model'' that provides the hierarchy of tests for a given system, starting with unit tests, which should be the most atomic and numerous tests.
...
\sugg{During the day, multiple developers commit changes that should be merged into a component's main branch and, at that point, a test suite run is queued for execution on a testing server, running a limited number of tests to ensure critical features are functional.
...
Here, we shall name this procedure, which happens at a fixed schedule, as Long Interval Regression Testing (LIRT).\footnote{\sugg{SIRT and LIRT are not terms used by the company, who asked to not disclose internal process names. These two acronyms are used here as a proxy, aiding legibility.}}}
...
\sugg{The investigated system is part of a software stack deployed to a delivered product.
Currently, there are two versions of the software in active development/maintenance mode and in use by customers.}
\todo{You asked to remove the whole paragraph, I tried to make it as non-descript as possible, see if this is sufficient.}
...
Reports indicate that \sugg{the two versions} have thousands of tests each.
\todo{Mentioning Erlang and simulators is ok?}
