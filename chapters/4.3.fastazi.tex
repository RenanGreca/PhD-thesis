...
\fs authors~\cite{miranda_fast} examined several possible variations of it
...
Indeed, our experiments show that the effectiveness and efficiency of the parallel and sequential approaches are statistically equivalent (according to the same analysis detailed in \Cref{sec:results}).
...
As an example, \Cref{fig:fastazi} contains sample outputs from \ek, \fs and \fz\footnote{This example is based on results of the experiments on \texttt{Chart v26}. Actual test names are omitted for clarity.}.
...
Algorithm \Cref{algo:fastazi} provides an abstract view of \ek and \fs\footnote{For a complete understanding of \ek and \fs, refer to~\cite{gligoricEk,miranda_fast}.}, and outlines how \fz works in practice.
