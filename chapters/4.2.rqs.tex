...
We evaluate \ek against \fs, and their combination (\fz) against either of them, considering first  their \textit{effectiveness} in  failure detection  (\nameref{rq:orch1}). Then, based on the real example shown in~\Cref{fig:fastazi}, we hypothesize that the potential gain in effectiveness of a combined approach could be better observed under a \textit{limited test budget} (\nameref{rq:orch2}). Finally we also compare their \textit{efficiency} (\nameref{rq:orch3}). Precisely,  we formulate the following research questions:
...
\paragraph{RQ\ref{chap:orchestration}.2}\label{rq:orch2} \textbf{How does a limited testing budget affect the effectiveness of the three approaches?}
While in \nameref{rq:orch1.1} and \nameref{rq:orch1.2} we compare \ek, \fs, and \fz without considering possible time constraints, with this RQ we aim at assessing whether, and how, testing under limited resources impacts each of the three approaches. 
